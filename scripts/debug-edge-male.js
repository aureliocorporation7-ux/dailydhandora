    ...
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            if (!completed) {
                ...
                ws.terminate();
                reject(new Error("Edge TTS Timeout (10s)"));
            }
        }, 10000);
...
(async () => {
    try {
        console.log("Starting Male Voice Verification...");
        const buffer = await edgeTts("नमस्ते, यह एक परीक्षण है।");
        fs.writeFileSync('test-edge-male.mp3', buffer);
        console.log(`✅ Success! Written ${buffer.length} bytes`);
    } catch (e) {
